...
pub async fn handle_anthropic(request: Request) -> Response {
    ...
    let upstream_url = format!("{}/v1/messages", route.base_url.trim_end_matches('/'));
...
async fn handle_non_streaming_response(
    ...
    let generator = AnthropicGenerator;
...
async fn handle_streaming_response(
    ...
    let parser: Option<Box<dyn FormatParser + Send>> = match target_format {
        ...
        "anthropic" => Some(Box::new(AnthropicParser)),
        "gemini" => Some(Box::new(crate::converter::parsers::gemini::GeminiParser)),
        _ => None,
    };
...
fn parse_target_response(
    ...
    match target_format {
        ...
        "anthropic" => AnthropicParser.parse_response(body),
