...
pub async fn handle_responses(request: Request) -> Response {
    ...
    let upstream_url = format!("{}/v1/responses", route.base_url.trim_end_matches('/'));
...
async fn handle_streaming_response(
    ...
    let parser: Option<Box<dyn FormatParser + Send>> = match target_format {
        ...
        "responses" => Some(Box::new(ResponsesParser)),
...
fn parse_target_response(
    ...
    match target_format {
        ...
        "responses" => ResponsesParser.parse_response(body),
        "anthropic" => {
