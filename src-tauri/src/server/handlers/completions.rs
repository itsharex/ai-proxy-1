...
pub async fn handle_completions(request: Request) -> Response {
    ...
    let api_key = match key_store.decrypt(&encrypted_key, &nonce) {
        Ok(k) => k,
        Err(e) => return ProxyError::KeyManagement(format!("Key decryption failed: {}", e)).into_response(),
    };

    // Set model from route
    ...
    // Build upstream URL
    let upstream_url = format!("{}/v1/chat/completions", route.base_url.trim_end_matches('/'));
    ...
    // Set auth header
    if route.auth_header == "Authorization" {
        req_builder = req_builder.header(
            "Authorization",
            format!("{} {}", route.auth_type, api_key),
        );
    } else {
    ...
    if is_stream {
        handle_streaming_response(resp, &route.target_format, "completions", status).await
    } else {
        handle_non_streaming_response(resp, &route.target_format, "completions", status).await
    }
}
...
async fn handle_streaming_response(
    ...
    let parser = match target_format {
        "completions" => {
            let p = Box::new(CompletionsParser) as Box<dyn FormatParser + Send>;
            Some(p)
        }
        "responses" => {
            let p = Box::new(crate::converter::parsers::responses::ResponsesParser)
                as Box<dyn FormatParser + Send>;
            Some(p)
        }
        "anthropic" => {
            let p = Box::new(crate::converter::parsers::anthropic::AnthropicParser)
                as Box<dyn FormatParser + Send>;
            Some(p)
        }
        "gemini" => {
            let p = Box::new(crate::converter::parsers::gemini::GeminiParser)
                as Box<dyn FormatParser + Send>;
            Some(p)
        }
        _ => None,
    ...
    let sse = Sse::new(stream.map(|item| item.map(|data| {
